...
export function runDoctor(cwd: string): void {
  ...
  if (major >= 20) {
    console.log(picocolors.green(`✔ Node.js version: ${nodeVersion} (Supported)`));
  } else {
    console.log(picocolors.red(`✖ Node.js version: ${nodeVersion} (Requires Node.js 20+)`));
  }
  ...
  try {
    const rgVersion = execSync('rg --version', { encoding: 'utf8' }).trim().split('\n')[0];
  ...
  if (providerDetails?.apiKey) {
    console.log(picocolors.green(`✔ API Key: Loaded for default provider "${defaultProvider}"`));
  } else {
    console.log(
      picocolors.red(
