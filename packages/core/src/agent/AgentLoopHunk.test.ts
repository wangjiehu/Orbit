...
describe('AgentLoop Hunk Acceptance Flow', () => {
  ...
  it('should accept all changes when user selects yes', async () => {
    ...
    const loop = new AgentLoop(testDir, dummyConfig, createMockProvider(), 'modify file', dummyInteraction);
    await loop.run();
  ...
  it('should apply selected hunks and reject other hunks', async () => {
    const askSelectSpy = vi.spyOn(Prompt, 'askSelect').mockResolvedValue('hunks');
    // Mock multi-select to return indices of hunks to apply (only apply Hunk 1)
    const askMultiSelectSpy = vi.spyOn(Prompt, 'askMultiSelect').mockResolvedValue(['0']);
