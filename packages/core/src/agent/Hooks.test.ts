...
describe('AgentLoop Hooks System', () => {
  const dummyConfig: OrbitConfig = {
    ...
    mcpServers: {},
    hooks: {
      preEdit: 'node -e "if (process.env.FAIL === \'true\') process.exit(1); console.log(\'pre-ok\')"',
      postEdit: 'node -e "if (process.env.FAIL_POST === \'true\') process.exit(1); console.log(\'post-ok\')"',
    },
  ...
  it('should run preEdit and postEdit hooks successfully', async () => {
    const loop = new AgentLoop(process.cwd(), dummyConfig, dummyProvider, 'test task', dummyInteraction);

    // Test runHook helper directly
    const resPre = await (loop as any).runHook(dummyConfig.hooks.preEdit!, 'dummy.txt');
    expect(resPre.ok).toBe(true);
    expect(resPre.output).toBe('pre-ok');

    // Test runHook failure
    process.env.FAIL = 'true';
    const resPreFail = await (loop as any).runHook(dummyConfig.hooks.preEdit!, 'dummy.txt');
  ...
  it('should substitute {file} placeholder in hooks', async () => {
    ...
    const res = await (loop as any).runHook(hookWithFile, 'dummy-test-file.txt');
